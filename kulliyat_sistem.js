window.addEventListener('load', function() {
    ...
    if (aranan) {
        ...
        document.getElementById('kapatBtn').onclick = () => { panel.remove(); window.history.replaceState({}, document.title, window.location.pathname); kirmiziButonEkle(); };
        ...
        function boya(node) {
            ...
            if (node.nodeType === 3) { if (node.data.match(regex)) { const span = document.createElement('span'); span.innerHTML = node.data.replace(regex, '$1<mark class="isaretli" style="background-color:#ffe066; color:#000; border-radius:2px; padding:0 2px;">$2</mark>'); node.parentNode.replaceChild(span, node); } }
            else if (node.nodeType === 1 && node.childNodes && !/(script|style|mark)/i.test(node.tagName)) { for (let i = 0; i < node.childNodes.length; i++) boya(node.childNodes[i]); }
        }
        boya(document.body);
        ...
        const git = (i) => {
            ...
            setTimeout(() => { hedef.scrollIntoView({ behavior: 'smooth', block: 'center' }); bulunanlar.forEach(el => { el.style.backgroundColor = "#ffe066"; }); hedef.style.backgroundColor = "#ff9632"; document.getElementById('sayac').innerText = `${aranan} (${i + 1}/${bulunanlar.length})`; }, 60);
        };
